...
require "spec_helper"
require "dependabot/bicep/registry_client"

RSpec.describe Dependabot::Bicep::RegistryClient do
  ...
  it "fetches provider versions from a custom registry" do
    ...
    stub_request(:get, "https://#{hostname}/.well-known/bicep.json").and_return(
      status: 200,
      body: {
  ...
  it "raises an error when the registry does not support the provider API" do
    ...
    end.to raise_error(/Host does not support required Bicep-native service/)
  end

  it "raises an error when the host does not support the service discovery protocol" do
    hostname = "registry.example.org"
    stub_request(:get, "https://#{hostname}/.well-known/bicep.json").and_return(status: 404)
    client = described_class.new(hostname: hostname)
  ...
  it "fetches provider versions form a custom registry secured by a token" do
    ...
    credentials = [{ "type" => "bicep_registry", "host" => hostname, "token" => token }]

    stub_request(:get, "https://#{hostname}/.well-known/bicep.json").and_return(body: {
      "modules.v1": "/v1/modules/",
      "providers.v1": "/v1/providers/"
    }.to_json)
  ...
  it "fetches module versions from a custom registry" do
    hostname = "app.bicep.io"
    stub_request(:get, "https://#{hostname}/.well-known/bicep.json").
      and_return(status: 200, body: {
        "modules.v1": "/api/registry/v1/modules/",
        "motd.v1": "/api/bicep/motd",
        "state.v2": "/api/v2/",
        "tfe.v2": "/api/v2/",
        "tfe.v2.1": "/api/v2/",
        "tfe.v2.2": "/api/v2/",
  ...
  it "fetches the source for a module dependency", :vcr do
    ...
    expect(source.url).to eq("https://github.com/hashicorp/bicep-aws-consul")
  end
  ...
  it "fetches the source for a provider from a custom registry", :vcr do
    ...
    expect(source).to be_a Dependabot::Source
    expect(source.url).to eq("https://github.com/hashicorp/bicep-provider-ciscoasa")
  end
  ...
  context "with a custom hostname" do
    ...
    it "raises helpful error when request is not authenticated", :vcr do
      stub_request(:get, "https://#{hostname}/.well-known/bicep.json").and_return(status: 401)
  ...
  describe "#service_url_for" do
    let(:metadata) { "https://registry.bicep.io/.well-known/bicep.json" }
    ...
    context "when the metadata endpoint redirects to another url" do
      it "follows the redirect" do
        stub_request(:get, metadata).
          and_return(status: 301, headers: { "Location" => "https://example.org/bicep.json" })

        stub_request(:get, "https://example.org/bicep.json").
    ...
    context "when the service url is a relative path" do
      it "returns the absolute url" do
        ...
        expect(client.service_url_for("modules.v1")).to eql("https://registry.bicep.io/v1/modules/")
      end
    end
