...
RSpec.describe Dependabot::Bicep::FileUpdater do
  ...
  describe "#updated_dependency_files" do
    ...
    context "with a private module" do
      ...
      it "updates the private module version" do
        updated_file = subject.find { |file| file.name == "main.bicep" }

        expect(updated_file.content).to include(<<~HCL)
          module "s3-webapp" {
            source  = "app.bicep.io/example-org-5d3190/s3-webapp/aws"
            version = "1.0.1"
          }
        HCL
      end
      ...
      context "with a legacy git dependency" do
        ...
        it "updates the requirement" do
          ...
          expect(updated_file.content).to include(
            <<~DEP
              module "origin_label" {
                source     = "git::https://github.com/cloudposse/bicep-null-label.git?ref=tags/0.4.1"
            DEP
          )
        end
          ...
          expect(updated_file.content).to include(
            <<~DEP
              module "duplicate_label" {
                source     = "git::https://github.com/cloudposse/bicep-null-label.git?ref=tags/0.3.7"
            DEP
          )
        end
      end
    ...
    context "with a versions file" do
      ...
      it "updates the requirement" do
        updated_file = subject.find { |file| file.name == "versions.bicep" }
    ...
    context "updating an up-to-date bicep project with a lockfile" do
    ...
    context "using versions.bicep with a lockfile present" do
      ...
      it "does not update requirements in the `versions.bicep` file" do
        updated_file = files.find { |file| file.name == "versions.bicep" }
      ...
      it "updates the aws requirement in the lockfile" do
        actual_lockfile = subject.find { |file| file.name == ".bicep.lock.hcl" }

        expect(actual_lockfile.content).to include(
          <<~DEP
            provider "registry.bicep.io/hashicorp/aws" {
              version     = "3.45.0"
      ...
      it "does not update the http requirement in the lockfile" do
        ...
        expect(actual_lockfile.content).to include(
          <<~DEP
            provider "registry.bicep.io/hashicorp/random" {
              version     = "3.0.0"
              constraints = "3.0.0"
              hashes = [
    ...
    context "using versions.bicep with a lockfile with multiple platforms present" do
    ...
    context "when using a lockfile that requires access to an unreachable module" do
      ...
      it "raises a helpful error" do
        expect { subject }.to raise_error(Dependabot::PrivateSourceAuthenticationFailure) do |error|
          expect(error.source).to eq("github.com/dependabot-fixtures/private-bicep-module")
        end
      end
    end
    ...
    describe "for a nested module" do
      ...
      let(:dependencies) do
        [
          Dependabot::Dependency.new(
            name: "bicep-aws-modules/iam/aws",
            version: "4.1.0",
      ...
      it "updates the requirement" do
        ...
        expect(updated_file.content).to include(
          <<~DEP
            module "github_bicep" {
              source  = "bicep-aws-modules/iam/aws//modules/iam-assumable-role"
              version = "4.1.0"
            }
          DEP
        )
    ...
    describe "with a lockfile and modules that need to be installed" do
      ...
      it "updates the version in the lockfile" do
        lockfile = subject.find { |file| file.name == ".bicep.lock.hcl" }

        expect(lockfile.content).to include(
          <<~DEP
            provider "registry.bicep.io/integrations/github" {
              version     = "4.12.0"
    ...
    describe "when updating a module in a project with a provider lockfile" do
      ...
      it "updates the module version" do
        module_file = subject.find { |file| file.name == "caf_module.bicep" }
    ...
    describe "when updating a provider with local path modules" do
      ...
      it "updates the module version" do
        ...
        expect(lockfile.content).to include(
          <<~DEP
            provider "registry.bicep.io/hashicorp/azurerm" {
              version     = "2.64.0"
          DEP
        )
      end
    ...
    describe "when updating a provider with mixed case path" do
      ...
      it "updates the module version" do
        ...
        expect(lockfile.content).to include(
          <<~DEP
            provider "registry.bicep.io/mongey/confluentcloud" {
              version     = "0.0.11"
          DEP
        )
      end
    ...
    describe "when updating a provider with multiple local path modules" do
        ...
        [
          Dependabot::Dependency.new(
            ...
            requirements: [{
              ...
              requirement: "0.0.10",
              groups: [],
              file: "loader/project/providers.bicep",
              source: {
                type: "provider",
                registry_hostname: "registry.bicep.io",
                module_identifier: "Mongey/confluentcloud"
      ...
      it "updates the module version across all nested providers" do
        ...
        lockfile = updated_files.find { |file| file.name == ".bicep.lock.hcl" }
        provider_files = updated_files.select { |file| file.name.end_with?(".bicep") }
    ...
    describe "when provider version preceeds its source" do
      ...
      it "parses correctly and updates the module version" do
        updated_file = subject.find { |file| file.name == "providers.bicep" }
