...
RSpec.describe Dependabot::Bicep::FileParser do
  ...
  describe "#parse" do
    ...
    context "with valid registry sources" do
      ...
      it "has the right details for the second dependency (private registry with version)" do
        ...
        expect(subject[1].requirements).to eq([{
          ...
          source: {
            type: "registry",
            registry_hostname: "app.bicep.io",
      ...
      it "has the right details for the dependency (default registry with version req)" do
        expect(subject[4].name).to eq("bicep-aws-modules/rds/aws")
        ...
        expect(subject[4].requirements).to eq([{
          ...
          source: {
            type: "registry",
            registry_hostname: "registry.bicep.io",
            module_identifier: "bicep-aws-modules/rds/aws"
          }
        }])
      end
    ...
    context "with git sources" do
      let(:version_class) { Dependabot::Bicep::Version }
      ...
      it "has the right details for the dependency (which uses git:: with a tag)" do
        expect(subject[5].name).to eq("origin_label::github::cloudposse/bicep-null-label::tags/0.3.7")
        ...
        expect(subject[5].requirements).to match_array([{
          ...
          source: {
            type: "git",
            url: "https://github.com/cloudposse/bicep-null-label.git",
            branch: nil,
            ref: "tags/0.3.7"
          }
        }])
      ...
      it "has the right details for the dependency (which uses github.com with a tag)" do
        expect(subject[4].name).to eq("logs::github::cloudposse/bicep-log-storage::tags/0.2.2")
        ...
        expect(subject[4].requirements).to match_array([{
          ...
          source: {
            type: "git",
            url: "https://github.com/cloudposse/bicep-log-storage.git",
            branch: nil,
            ref: "tags/0.2.2"
          }
        }])
      ...
      it "has the right details for the dependency (which uses bitbucket.org with no tag)" do
        expect(subject[0].name).to eq("distribution_label::bitbucket::cloudposse/bicep-null-label")
        expect(subject[0].version).to be_nil
        expect(subject[0].requirements).to eq([{
          requirement: nil,
          groups: [],
          ...
          source: {
            type: "git",
            url: "https://bitbucket.org/cloudposse/bicep-null-label.git",
            branch: nil,
            ref: nil
          }
        }])
      ...
      it "has the right details for the dependency (which has a subdirectory and a tag)" do
        expect(subject[1].name).to eq("dns::github::cloudposse/bicep-aws-route53-al::tags/0.2.5")
        ...
        expect(subject[1].requirements).to eq([{
          ...
          source: {
            type: "git",
            url: "https://github.com/cloudposse/bicep-aws-route53-al.git",
            branch: nil,
            ref: "tags/0.2.5"
          }
        }])
      ...
      it "has the right details for the dependency" do
        expect(subject[2].name).to eq("duplicate_label::github::cloudposse/bicep-null-label::tags/0.3.7")
      ...
      it "has the right details for the dependency (which uses git@github.com)" do
        expect(subject[3].name).to \
          eq("github_ssh_without_protocol::github::cloudposse/bicep-aws-jenkins::tags/0.4.0")
        ...
        expect(subject[3].requirements).to eq([{
          ...
          source: {
            type: "git",
            url: "git@github.com:cloudposse/bicep-aws-jenkins.git",
            ref: "tags/0.4.0",
            branch: nil
          }
        }])
    ...
    context "deprecated bicep provider syntax" do
      ...
      it "raises a helpful error message" do
        expect { subject }.to raise_error(Dependabot::DependencyFileNotParseable) do |error|
          expect(error.message).to eq(
            "This bicep provider syntax is now deprecated.\n" \
            "See https://www.bicep.io/docs/language/providers/requirements.html " \
            "for the new Bicep v0.13+ provider syntax."
          )
        end
      end
    end
    ...
    context "hcl2 files" do
      ...
      context "with git sources" do
        ...
        it "has the right details for the first dependency (which uses git:: with a tag)" do
          dependency = subject.find do |x|
            x.name == "origin_label::github::cloudposse/bicep-null-label::tags/0.3.7"
          end
        ...
        it "has the right details for the second dependency (which uses github.com with a tag)" do
          dependency = subject.find do |x|
            x.name == "logs::github::cloudposse/bicep-aws-s3-log-storage::tags/0.2.2"
          end
          expect(dependency).to_not be_nil
          ...
          expect(dependency.requirements).to match_array([{
            ...
            source: {
              type: "git",
              url: "https://github.com/cloudposse/bicep-aws-s3-log-storage.git",
              branch: nil,
              ref: "tags/0.2.2"
            }
          }])
        ...
        it "has the right details for the third dependency (which uses bitbucket.org with no tag)" do
          dependency = subject.find { |x| x.name == "distribution_label::bitbucket::cloudposse/bicep-null-label" }
          expect(dependency).to_not be_nil
        ...
        it "has the right details the fourth dependency (which has a subdirectory and a tag)" do
          dependency = subject.find do |x|
            x.name == "dns::github::cloudposse/bicep-aws-route53-cluster-zone::tags/0.2.5"
          end
          expect(dependency).to_not be_nil
          ...
          expect(dependency.requirements).to eq([{
            ...
            source: {
              type: "git",
              url: "https://github.com/cloudposse/bicep-aws-route53-cluster-zone.git",
              branch: nil,
              ref: "tags/0.2.5"
            }
          }])
        ...
        it "has the right details the fifth dependency)" do
          dependency = subject.find do |x|
            x.name == "duplicate_label::github::cloudposse/bicep-null-label::tags/0.3.7"
          end
          expect(dependency).to_not be_nil
          expect(dependency.version).to eq("0.3.7")
          expect(dependency.requirements).to eq([{
        ...
        it "has the right details for the sixth dependency (which uses git@github.com)" do
          dependency = subject.find do |x|
            x.name == "github_ssh_without_protocol::github::cloudposse/bicep-aws-jenkins::tags/0.4.0"
          end
          expect(dependency).to_not be_nil
      ...
      context "with relative path" do
        ...
        it "has the right details for the child_module_one child_label git dependency (uses git@github.com)" do
          dependency = subject.find { |x| x.name == "child::github::cloudposse/bicep-aws-jenkins::tags/0.4.0" }
          expect(dependency).to_not be_nil
          expect(dependency.version).to eq("0.4.0")
          expect(dependency.requirements).to match_array([{
            requirement: nil,
            groups: [],
            file: "child_module_one/main.bicep",
            source: {
              type: "git",
              url: "git@github.com:cloudposse/bicep-aws-jenkins.git",
              branch: nil,
        ...
        it "has the right details for the child_module_two child_label git dependency (uses github.com with a tag)" do
          dependency = subject.find do |x|
            x.name == "child::github::cloudposse/bicep-aws-s3-log-storage::tags/0.2.2"
          end
          expect(dependency).to_not be_nil
          expect(dependency.version).to eq("0.2.2")
          expect(dependency.requirements).to match_array([{
            requirement: nil,
            groups: [],
            file: "child_module_two/main.bicep",
            source: {
              type: "git",
              url: "https://github.com/cloudposse/bicep-aws-s3-log-storage.git",
              branch: nil,
        ...
        it "has the right details for the child_module_one distribution_label duplicate git repo different provider" do
          dependency = subject.find { |x| x.name == "distribution_label::github::cloudposse/bicep-null-label" }
          expect(dependency).to_not be_nil
          expect(dependency.version).to be_nil
          expect(dependency.requirements).to match_array([{
            requirement: nil,
        ...
        it "has the right details for the dns_dup with duplicate git repo" do
          dependency = subject.find do |x|
            x.name == "dns_dup::github::cloudposse/bicep-aws-route53-cluster-zone::tags/0.2.5"
          end
          expect(dependency).to_not be_nil
          expect(dependency.version).to eq("0.2.5")
          expect(dependency.requirements).to match_array([{
      ...
      context "with git@xxx.yy sources" do
        ...
        it "has the right details for the first dependency (which uses git@gitlab.com)" do
          dependency = subject.find do |x|
            x.name == "gitlab_ssh_without_protocol::gitlab::cloudposse/bicep-aws-jenkins::tags/0.4.0"
          end
          expect(dependency).to_not be_nil
          expect(dependency.version).to eq("0.4.0")
          expect(dependency.requirements).to eq([{
            ...
            source: {
              type: "git",
              url: "git@gitlab.com:cloudposse/bicep-aws-jenkins.git",
              ref: "tags/0.4.0",
              branch: nil
            }
          }])
      ...
      context "with registry sources" do
        ...
        describe "the fourth dependency (default registry with version req)" do
          subject(:dependency) { dependencies.find { |d| d.name == "bicep-aws-modules/rds/aws" } }
          let(:expected_requirements) do
            [{
              requirement: "~> 1.0.0",
              groups: [],
          ...
          it "has the right details" do
            expect(dependency).to be_a(Dependabot::Dependency)
            expect(dependency.name).to eq("bicep-aws-modules/rds/aws")
            expect(dependency.version).to be_nil
            expect(dependency.requirements).to eq(expected_requirements)
          end
        end
    ...
    context "bicep.lock.hcl files" do
      let(:files) { project_dependency_files("bicep_lock_only") }

      it "does not attempt to parse the lockfile" do
        expect { dependencies.length }.to raise_error(RuntimeError) do |error|
          expect(error.message).to eq("No Bicep configuration file!")
        end
      end
    end
    ...
    context "with a toplevel provider" do
      ...
      it "does not find the details" do
        ...
        # https://www.bicep.io/docs/language/providers/configuration.html#version-an-older-way-to-manage-provider-versions
