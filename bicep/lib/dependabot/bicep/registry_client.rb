...
module Dependabot
  module Bicep
    # Bicep::RegistryClient is a basic API client to interact with a
    # bicep registry: https://www.bicep.io/docs/registry/api.html
    class RegistryClient
      ARCHIVE_EXTENSIONS = %w(.zip .tbz2 .tgz .txz).freeze
      PUBLIC_HOSTNAME = "registry.bicep.io"

      def initialize(hostname: PUBLIC_HOSTNAME, credentials: [])
        @hostname = hostname
        @tokens = credentials.each_with_object({}) do |item, memo|
          memo[item["host"]] = item["token"] if item["type"] == "bicep_registry"
        end
      end

      # rubocop:disable Metrics/PerceivedComplexity
      # See https://www.bicep.io/docs/modules/sources.html#http-urls for
      # details of how Bicep handle HTTP(S) sources for modules
      def self.get_proxied_source(raw_source) # rubocop:disable Metrics/AbcSize
        ...
        url = raw_source.split(%r{(?<!:)//}).first + "?bicep-get=1"
        ...
        return response.headers["X-Bicep-Get"] if response.headers["X-Bicep-Get"]

        doc = Nokogiri::XML(response.body)
        doc.css("meta").find do |tag|
          tag.attributes&.fetch("name", nil)&.value == "bicep-get"
      ...
      # "hashicorp/aws"
      # @return [Array<Dependabot::Bicep::Version>]
      ...
      def source(dependency:)
        ...
        case type
        # https://www.bicep.io/internals/module-registry-protocol#download-source-code-for-a-specific-module-version
        when "module", "modules", "registry"
          ...
          source_url = response.headers.fetch("X-Bicep-Get")
      ...
      # https://www.bicep.io/docs/internals/remote-service-discovery.html
      #
      # @param service_key [String] the service type described in https://www.bicep.io/docs/internals/remote-service-discovery.html#supported-services
      # @param return String
      ...
      def service_url_for(service_key)
        ...
        raise Dependabot::PrivateSourceAuthenticationFailure, "Host does not support required Bicep-native service"
      end
      ...
      def services
        @services ||=
          begin
            response = http_get(url_for("/.well-known/bicep.json"))
